...
export default function HoaDonDaBanTab({ idNV }) {
  ...
  const [dataSource, setDataSource] = useState(
    HoaDons.filter((HoaDon) => HoaDon.idNV === idNV)
  );

  useEffect(() => {
    dispatch(getHoaDons.getHoaDonsRequest());
    if (HoaDons) {
      setDataSource(HoaDons.filter((HoaDon) => HoaDon.idNV === idNV));
  ...
  return (
    <div>
      ...
      >
        ...
        >
          Tổng số hóa đơn đã bán: <br />
          Tổng tiền đã thu được: <br />
        </label>
